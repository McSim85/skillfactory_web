...
def parse_connection_string(connection_string):
    ...
    final = {'dialect': '',
             ...
             'port': '',
             'database': ''}
    proto, dbinfo = connection_string.split('://')
    # parse protocol
...
#            final['dialect'] = dialect
    ...
    if '+' in proto:
        final['dialect'], final['driver'] = proto.split('+')
    else:
        final['dialect'] = proto
    
    if '/' in dbinfo:
        otherinfo, final['database'] = dbinfo.split('/', 1)
    else:
        otherinfo = dbinfo
...
#    print('otherinfo is ', otherinfo)
    if otherinfo:
        if '@' in otherinfo:
            user, host = otherinfo.split('@')
            #parse hostname
            if ':' in host:
                final['host'], final['port'] = host.split(':')
            else:
                final['host'] = host
            #parse username
            if ':' in user:
                final['username'], final['password'] = user.split(':')
            else:
                final['username'] = user  
            
        else:
            if ':' in otherinfo:
                final['username'], final['password'] = otherinfo.split(':')
            else:
